...
export function Footer({ settings }: { settings: SiteSettings }) {
  ...
  return <footer className="footer"><div className="container">
    <div className="grid gap-10 md:grid-cols-[2fr_1fr_1fr]"><div>
      {logo ? <Image src={urlFor(logo).width(300).height(300).fit('max').auto('format').url()} alt={logo.alt || settings.siteName} width={44} height={44} className="brand-uploaded-logo footer-logo" /> : <div className="brand text-white"><span className="brand-mark"><Network size={21} /></span><span className="brand-copy">Führungsfrauen<small className="text-white">Raum Heilbronn</small></span></div>}
