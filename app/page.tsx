...
export default async function Home() {
  ...
  const featureImage = content?.featureImage && typeof content.featureImage === 'object' ? content.featureImage as SanityImageSource & { alt?: string } : null;
  return <>
    ...
    <section className="feature-band"><div className="container grid gap-10 md:grid-cols-2 md:items-center"><div className={`image-panel ${featureImage ? 'has-image' : ''}`}>{featureImage && <Image src={urlFor(featureImage).width(1200).height(900).fit('crop').auto('format').url()} alt={featureImage.alt || ''} fill sizes="(min-width: 768px) 45vw, 100vw" className="object-cover" />}<p className="image-panel-label">Netzwerk ist Beziehung.</p></div><div className="md:pl-12"><p className="eyebrow">Das Netzwerk</p><h2 className="h2 mt-6">Vielfältige Wege. Ein gemeinsamer Austausch.</h2><p className="mt-7 max-w-lg text-lg leading-relaxed text-[var(--muted)]">Unternehmerinnen, Führungskräfte, Gründerinnen, Wissenschaftlerinnen und Frauen auf dem Weg in Führung treffen hier aufeinander. Teil des Netzwerks zu sein, setzt keine Vereinsmitgliedschaft voraus.</p><div className="mt-8"><Button href="/netzwerk">Netzwerk kennenlernen</Button></div></div></div></section>
